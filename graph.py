...
def run_command(cmd: str):
        ...
        Executes a Windows shell command and returns the output or error.
        """
        cmd = cmd.replace("\\n", "`n")
        cmd = cmd.replace("&&", ";")
        cmd = cmd.replace("/", "\\")
...
def chatbot(state: State):
    system_prompt = SystemMessage(content="""
        ...
        tools you choose the correct tool and execute the commands on windows powershell.
        Rules:
        - always use `n to create newline character.make sure correct windows commands to execute on command line                        
